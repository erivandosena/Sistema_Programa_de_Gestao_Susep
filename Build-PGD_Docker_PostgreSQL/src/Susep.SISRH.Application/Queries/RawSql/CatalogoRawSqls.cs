            ...
            get
            {
                return @"
                    SELECT  i.itemCatalogoId 
							,i.titulo         
                            --,i.titulo + [ProgramaGestao].[IIF](i.complexidade IS NULL OR i.complexidade = '', ''::text, ' - ' + i.complexidade) as tituloCompleto
                            ,i.titulo + [ProgramaGestao].[IIF](i.complexidade IS NULL OR i.complexidade = '', ''::text, ' - ' + i.complexidade) + '  (pres.:' + cast(i.tempoPresencial as varchar(5)) + 'h / rem.:' + cast(coalesce(i.tempoRemoto,0) as varchar(5)) + 'h)' as tituloCompleto
                            ,i.complexidade 
