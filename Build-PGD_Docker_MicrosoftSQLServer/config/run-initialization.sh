...
exists=$(/opt/mssql-tools/bin/sqlcmd -l 30 -S localhost -h-1 -V1 -U sa -P $SA_PASSWORD -Q "SET NOCOUNT ON SELECT case when name is not null then 1 else 0 end as existe from master.sys.databases where name = 'pgd_staging'")
if [[ $exists -eq 1 ]];
then
    echo "⏳ Realizando backup do banco de dados existente"
    # Executa o script de backup do banco de dados.
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P $SA_PASSWORD -d pgd_staging -i backup-database.sql
else
    ...
    /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P $SA_PASSWORD -d pgd_staging -i restore-database.sql
fi
